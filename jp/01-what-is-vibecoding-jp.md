...
VibeFastはフルスタックテンプレートで、フロントエンドに **Remix**、バックエンドに **Cloudflare Workers** を使用している。
...
Cloudflareには300以上のエッジノードがあり、VibeFastをデプロイすれば世界中のユーザーが高速にアクセスできる。
...
Workersの用途は広い——バックエンドAPIロジックを実行することも、フロントエンドのSSR（サーバーサイドレンダリング）を実行することもできる。VibeFastのアーキテクチャでは、フロントエンド（Remix）とバックエンドAPIはそれぞれ独立したWorkerで実行され、2つのWorkerはService Bindingを通してCloudflare内部で通信する。
...
VibeFastでは、フロントエンド（Remix）とバックエンド（Workers API）は2つの独立したサービスだ。従来の方法では、フロントエンドは公開ネットワーク経由でバックエンドAPIを呼び出す必要があり、CORS問題（クロスオリジンリクエストのセキュリティ制限）が発生して追加設定が必要になる。
...
これらのコードはプロダクトのコアではないが、毎回書き直すのは時間の無駄だ。VibeFastのようなテンプレートはこのboilerplateをあらかじめ用意しておき、「プロダクト機能」から直接始められるようにする。
...
VibeFastのようなテンプレートは通常JWT認証が設定済みで、ゼロから実装する必要はない。

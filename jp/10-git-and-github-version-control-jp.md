...
**クローン（Clone）**  
GitHubのリポジトリをローカルにコピーする操作。VibeFastの`git clone`がこれだ。
...
VibeFastには設定済みの`.gitignore`が含まれているが、新しい機密ファイルを作った場合は自分で追加する。
...
VibeFastを購入した場合は既にリポジトリがある。新しいプロジェクトをゼロから始める場合：
